create or replace function public.initialize_mercadopago_ticket_payment(p_ticket_id uuid)
...
begin
  ...
  if not found or not private.user_is_company_member(v_ticket.company_id) then
    raise exception 'not_found_or_forbidden';
  end if;
  if v_ticket.status <> 'closed' or v_ticket.final_cost is null or v_ticket.final_cost <= 0 then
    raise exception 'ticket_not_payable';
  end if;

  select * into v_payment
  from public.payments
  where ticket_id = p_ticket_id and provider = 'mercado_pago'
  ...
  if not found then
    ...
    values(v_ticket.company_id,p_ticket_id,'mercado_pago','mp:' || p_ticket_id::text,v_ticket.final_cost,'CLP','pending',(select auth.uid()))
...
create or replace function public.finalize_mercadopago_ticket_payment(
  p_payment_id uuid,
...
declare
  ...
  v_payable uuid;
  v_platform numeric;
  v_specialist_amount numeric;
  v_commission_percent numeric := 20;
begin
  if current_user not in ('service_role','postgres') then raise exception 'forbidden'; end if;
  select * into v_payment from public.payments where id=p_payment_id and provider='mercado_pago' for update;
  if not found then raise exception 'payment_not_found'; end if;
  if v_payment.status='captured' then return; end if;
  if p_provider_status <> 'approved' then
    update public.payments set status=case when p_provider_status in ('pending','in_process') then 'authorized' else 'failed' end,
      provider_reference=p_provider_reference where id=p_payment_id;
    return;
  end if;
  ...
  v_platform:=round(v_payment.amount*v_commission_percent/100,2);
  v_specialist_amount:=v_payment.amount-v_platform;
  ...
  insert into public.ledger_transactions(payment_id,description,created_by) values(p_payment_id,'Captura Mercado Pago de ticket',v_payment.created_by)
    on conflict(payment_id) do nothing returning id into v_tx;
  if v_tx is not null then
    insert into public.ledger_entries(transaction_id,account_id,direction,amount,currency_code)
    values(v_tx,v_cash,'debit',v_payment.amount,'CLP'),(v_tx,v_revenue,'credit',v_platform,'CLP'),(v_tx,v_payable,'credit',v_specialist_amount,'CLP');
  end if;
end;
$$;

revoke all on function public.initialize_mercadopago_ticket_payment(uuid) from public,anon;
grant execute on function public.initialize_mercadopago_ticket_payment(uuid) to authenticated;
revoke all on function public.finalize_mercadopago_ticket_payment(uuid,text,numeric,text) from public,anon,authenticated;
grant execute on function public.finalize_mercadopago_ticket_payment(uuid,text,numeric,text) to service_role;
